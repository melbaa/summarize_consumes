...
def test_sanity1():
    # check all items in name2consumable that have a price are in name2itemid and the itemid matches
    found = 0
    for name, consumable in NAME2CONSUMABLE.items():
        price_comp = consumable.get_components(PriceComponent)[0]
        if not price_comp:
            continue
        if not isinstance(price_comp.price, DirectPrice):
            continue
        if price_comp.price.itemid != NAME2ITEMID[name]:
            raise ValueError(f"Price for {name} does not match itemid")
        found += 1
    assert found > 0
